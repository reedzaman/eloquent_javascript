...
// given a char code, returns the language object 
// where the char belongs
function in_which_script(char_code){
  for(elt of theFile){
    if(elt.ranges.some( ([from, to])=>{
...
/*
returns the set of scripts and how many elements of the data set belong to every particular
script
it's parameter takes a function that returns a string which is counted for appearing
every time
*/
function count_by(data_set , func){
  let container = [];
  for(element of data_set){
    result = func(element);
    let index = container.findIndex( x => x.name == result);
    if(index == -1){
      container.push({name: result, count: 1});
    }
    else container[index].count++;
  }
  return container;
}


// returns an array of objects {script_name: apearance_parcentage}
function text_stat(data){
  let counted = count_by(data, x => in_which_script(x.codePointAt(0)).name);
  let total = counted.map(x => x.count).reduce((total, curr) => total + curr);
  return counted.map( x =>{
    return `${x.name} : ${Math.round(x.count * 100 / total)}%`;
  });
}

console.log(text_stat("somewhereOverTheRainbowএখানেআকাশনীল"));
// -> [ 'Latin : 66%', 'Bengali : 34%' ]
