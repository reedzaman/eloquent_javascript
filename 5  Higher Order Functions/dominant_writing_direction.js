...
/*
..takes a char code
..returns the script it belongs to
..returns null in case the char code belong to nothing
*/
function in_which_script(char_code){
  let script;
  for(let element of theFile){
    if(element.ranges.some(([from, to])=>{
      return char_code>= from && char_code < to ;
    })){
      return element;
    }
  }
...
..each object has a writing direction name and number of ouccurence
*/
function find_all_writing_direction(str){
  let directions = [];
  for(let element of str){
    let script = in_which_script(element.codePointAt(0));
    if(script){
      let result = directions.findIndex( x=> x.name == script.direction);
      if(result != -1){
        directions[result].count++;
      }
      else {
        directions.push({name: script.direction, count: 1});
      }
    }
  }
  return directions;
...
/*
.. takes a string
.. returns which is the dominant writing direction
   among the charecters
*/
function dominant_writing_direction(str){
  let result = find_all_writing_direction(str).reduce((curr , dir) => {
    if(dir.count > curr.count){
      return curr = dir;
    }
  });
  return result;
...
/* 
.. here i staffed some latin, arabic and, mongolian text 
   and checked which is the dominant writing direction
*/
console.log(dominant_writing_direction("sعندميريدالعالمfsᠬᠦᠮᠦᠨ ᠪᠦᠷ ᠲᠥᠷᠥᠵᠦ ᠮᠡᠨᠳda"));
// -> {name: 'ttb', count: 18}
